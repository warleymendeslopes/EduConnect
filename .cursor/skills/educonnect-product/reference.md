...
- **Desempenho (relatórios na sala):** `scripts/007_classroom_performance_stats.sql` — função `class_activity_score_stats(p_classroom_id uuid)` (SECURITY DEFINER): médias agregadas por atividade avaliativa (`settings ? 'exam'`, atividade não rascunho, entregas `enviado` com `score_total`), acessível a professor da sala ou aluno membro, sem expor notas individuais de terceiros via RLS. Agregações adicionais do professor em `app/actions/classroom-performance.ts` (`getClassroomPerformanceForProfessor`, opções `studentsPage` / `studentsPageSize`, padrão 25 alunos por página; gráfico de distribuição em faixas fixas). Aluno: `getMyPerformanceInClassroom` (RPC + próprias entregas); comparação «acima/abaixo da média» por atividade. Professor: `getProfessorStudentOverview(studentId)` — salas em comum + mesmo bloco de métricas por turma (`loadStudentSelfPerformanceForClassroom`). UI: [`components/dashboard/classroom-performance-panel.tsx`](components/dashboard/classroom-performance-panel.tsx) (separador Desempenho), [`components/dashboard/aluno-performance-panel.tsx`](components/dashboard/aluno-performance-panel.tsx) (aba Desempenho aluno `?tab=desempenho`; visão professor com `audience="professor"` em `/dashboard/professor/alunos/[studentId]`). Aplicar `007` após `005`.
- **Plano de estudos (aluno):** `scripts/008_student_planner_personal_tasks.sql` — tabela `student_planner_personal_tasks` (`student_id` → `profiles`, `title`, `notes`, `scheduled_on` **date**, `is_done`, `done_at`); RLS só dono (`student_id = auth.uid()`); insert exige `profiles.user_type = 'aluno'`. Sem misturar com `classroom_activities`. Leitura da semana em [`app/actions/student-planner.ts`](app/actions/student-planner.ts): `getPlannerWeek` agrega atividades de **todas** as salas do aluno (`classroom_activities` com `status <> 'rascunho'`), coloca cada atividade no dia de `due_at` ou, se null, `starts_at`; se ambos null, lista em **«Sem data no calendário»**. Tarefas pessoais: CRUD + toggle concluído (`createPersonalPlannerTask`, `updatePersonalPlannerTask`, `deletePersonalPlannerTask`, `togglePersonalPlannerTaskDone`). UI: [`app/dashboard/aluno/plano/page.tsx`](app/dashboard/aluno/plano/page.tsx) (server) + [`plano-estudos-client.tsx`](app/dashboard/aluno/plano/plano-estudos-client.tsx); query `?semana=YYYY-MM-DD` (segunda-feira da semana visível). Itens de **sala**: só leitura + link «Abrir atividade»; **pessoais**: editar/remover/concluir. Tipos partilhados em [`lib/student-planner/types.ts`](lib/student-planner/types.ts). **Dias de sequência (streak):** [`lib/student-planner/streak.ts`](lib/student-planner/streak.ts) — `stats.streakDays` em `getPlannerWeek`; conta dias civis consecutivos em `America/Sao_Paulo` com pelo menos uma tarefa pessoal concluída (`done_at`) ou entrega `enviado` (`submitted_at` em `classroom_activity_submissions`); olhar ~400 dias para trás; se hoje ainda sem atividade, a sequência pode terminar em ontem. Aplicar `008` após `001`.
...
| Aluno | `/dashboard/aluno` (feed), `/dashboard/aluno/plano`, `/dashboard/aluno/salas`, `/dashboard/aluno/salas/[id]` (`?tab=atividades` \| `materiais` \| `visao` \| `desempenho`), `/dashboard/aluno/salas/[id]/atividades/[activityId]` (detalhe + descrição HTML sanitizada), `/dashboard/aluno/tutor`, `/dashboard/aluno/explorar` |
| Professor | `/dashboard/professor` (feed), `/dashboard/professor/criar`, `/dashboard/professor/salas`, `/dashboard/professor/salas/[id]`, `/dashboard/professor/alunos` (lista + link no nome), `/dashboard/professor/alunos/[studentId]` (desempenho do aluno só nas turmas do professor) |
...
- Professor: `/analise`, `/revisoes`, `/perfil`; estado **cadastro pendente** (banner + bloqueio criar conteúdo/sala) se ainda não houver campo/workflow no banco.
...
Página `/dashboard/aluno/plano?semana=YYYY-MM-DD` (opcional; padrão semana corrente, segunda a domingo). Calendário semanal com atividades das salas (somente leitura, link para o detalhe) e tarefas criadas pelo aluno (CRUD). Stats derivados: **dias de sequência** (streak: tarefa pessoal concluída ou entrega no dia, fuso Brasília), tarefas pessoais concluídas, envios de atividades da semana, percentagem global de itens concluídos; horas estudadas não rastreadas. Barra «Progresso por disciplina» usa envios por `subject` na semana. Sidebar lista **avaliações da semana** (atividades com `settings.exam`) e tarefas pessoais. Atividades sem prazo nem início aparecem no aviso **Sem data no calendário**. Ajuste por IA: fora do MVP.
...
Página [`/dashboard/professor/alunos`](app/dashboard/professor/alunos/page.tsx): lista **alunos distintos** que participam de pelo menos uma sala do professor; por aluno, links para cada turma (`classroom_members` + `classrooms`) e **link no nome** para [`/dashboard/professor/alunos/[studentId]`](app/dashboard/professor/alunos/[studentId]/page.tsx) — desempenho (mesmas métricas que o aluno vê na aba Desempenho) **por turma** em que professor e aluno coincidem. Query `?q=` (busca por nome), `?page=` (paginação, 25 por página). Barra de busca com sugestões (autocomplete) via mesma action. Action [`listStudentsAcrossClassroomsForProfessor`](app/actions/classrooms.ts) agrega no servidor e aplica filtro + slice; turmas muito grandes podem exigir RPC no futuro. Ao remover aluno de uma sala, `revalidatePath` inclui esta rota.
...
/dashboard/aluno/plano     → Plano (?semana=YYYY-MM-DD segunda da semana)
...
/dashboard/professor/alunos    → Alunos (agregado por turmas do professor)
/dashboard/professor/alunos/:studentId → Detalhe aluno (desempenho nas turmas do professor)
